...
const StoriesComponent: React.FC<StoriesComponentProps> = ({
  ...
  useEffect(() => {
    if (isViewingStory) {
      if (activeStoryIndex === -1 && myStories.length > 0) {
        // Viewing my stories
        setStoryIndicators(
          Array.from({ length: myStories.length }, (_, i) =>
        ...
        setStoryIndicators(
          Array.from(
            { length: stories[activeStoryIndex].stories.length },
            (_, i) =>
              i === activeStoryItemIndex
                ? progress
                : i < activeStoryItemIndex
  ...
  const handleStoryCreated = (story: { image: string; caption: string }) => {
      ...
      <div className="w-full max-w-sm md:max-w-lg lg:max-w-xl mx-auto py-2 sm:py-4 px-2 sm:px-4 transition-colors duration-200">
        ...
        >
          ...
          <div className="flex-shrink-0 p-1" onClick={handleMyStoryClick}>
            <div
              className={`relative ${storySize} rounded-full p-0.5 cursor-pointer transform transition-transform hover:scale-105 ${
                myStories.length > 0
            ...
            <p
              className={`${storyTextSize} text-center mt-1 text-gray-800 dark:text-gray-300`}
            >
          ...
          {stories.map((story, index) => (
            ...
            >
              ...
              <p
                className={`${storyTextSize} text-center mt-1 text-gray-800 dark:text-gray-300 truncate max-w-16`}
              >
                {story.user}
              </p>
            </div>
